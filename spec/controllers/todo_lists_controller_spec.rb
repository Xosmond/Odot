...
RSpec.describe TodoListsController, type: :controller do
  let(:valid_attributes) {{"titulo" => "Nuevo Titulo", "descripcion"=>"Mi nueva descripcion"}}
  let(:invalid_attributes) {{"titulo" => "N", "descripcion"=>"Mi"}}
  let(:valid_session) { {} }
  ...
  describe "GET #index" do
    it "Listar las listas del usuario." do
      todo_list = usuario.todo_lists.create! valid_attributes
      get :index, {}, valid_session
      expect(todo_list).to eq(todo_list)
      expect(assigns(:todo_lists).map(&:usuario)).to eq([usuario])
    end
  end
  ...
  describe "GET #new" do
    it "assigns a new todo_list as @todo_list" do
      todo_list = usuario.todo_lists.new
      get :new, {}, valid_session
      expect(todo_list).to be_a_new(TodoList)
      expect(todo_list.usuario).to eq(usuario)
    end
  end
  ...
  describe "PUT #update" do
    context "with valid params" do
      let(:new_attributes){{"titulo" => "Titulo Actualizado", "descripcion"=>"Mi nueva descripcion Actualizado"}}

      it "updates the requested todo_list" do
        todo_list = usuario.todo_lists.create! valid_attributes
        put :update, {:id => todo_list.to_param, :todo_list => new_attributes}, valid_session
        todo_list.reload
        expect(assigns(:todo_list).titulo).to eq("Titulo Actualizado")
        expect(assigns(:todo_list).descripcion).to eq("Mi nueva descripcion Actualizado")
      end
